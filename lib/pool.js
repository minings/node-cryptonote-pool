...
setInterval(function() {
        ...
        for (var minerId in connectedMiners) {
            var miner = connectedMiners[minerId]; << << << < HEAD
            if (now - miner.lastBeat > timeout) {
                log('debug', 'Miner Cleaner', 'Disconnected miner ' + miner.login); === === =
                if (now - miner.lastBeat > timeout) {
                    log('info', logSystem, 'Miner timed out and disconnected %s', [miner.login]); >>> >>> > upstream / master
    ...
    function processShare(miner, job, blockTemplate, nonce, resultHash) {
        ...
        if (hashDiff.ge(blockTemplate.difficulty)) {
            ...
            log('error', logSystem, 'Rejected low difficulty share of %s', [hashDiff.toString()]);
            return false;
        } else {
            recordShareData(miner, job, hashDiff.toString(), false, null, shareType);
        }
    ...
    function handleMinerMethod(method, params, ip, portData, sendReply, pushMessage) {
        ...
        switch (method) {
            case 'login':
                ...
                if (!utils.isValidAddress(params.login, config.poolServer.poolAddress)) {
            ...
            net.createServer(function(socket) {
                ...
                socket.on('data', function(d) {
                    ...
                    if (dataBuffer.indexOf('\n') !== -1) {
                        ...
                        messages.forEach(function(message) {
                            ...
                            try {
                                ...
                                log('warn', logSystem, 'Malformed message %s', [message]);
                                socket.destroy();
                                return;
                            }
                    ...
                    if (err.code !== 'ECONNRESET')
                        log('warn', logSystem, 'Socket error %j', [err]);
                }).on('close', function() {
