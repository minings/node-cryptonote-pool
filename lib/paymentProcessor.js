    ...
    async.waterfall([
        ...
        function(balances, callback) {
            ...
            for (var worker in payments) {
                ...
                if (amount >= config.payments.maxAmount) {
                    transferCommands[commandIndex].rpc.destinations.push({
                        amount: config.payments.maxAmount,
                        address: worker
                    });
                    transferCommands[commandIndex].redis.push(['hincrby', config.coin + ':workers:' + worker, 'balance', -config.payments.maxAmount]);
                    transferCommands[commandIndex].redis.push(['hincrby', config.coin + ':workers:' + worker, 'paid', config.payments.maxAmount]);
